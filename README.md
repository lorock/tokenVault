...
部署目标为 **Cloudflare Pages + 自定义域名根目录**（官方站点 `tokenvault.sre.wang`，已 ICP 备案、国内微信等环境可正常访问；`tokenvault.xubaojin.com` 为国际跳转域名，自动重定向至此），`vite.config.js` 的 `base` 默认 `/`，无需任何配置即可根目录部署。
...
本工具官方站点为 **tokenvault.sre.wang**（Cloudflare Pages 根目录部署，已 ICP 备案、国内微信等环境可正常访问）；`tokenvault.xubaojin.com` 为国际跳转域名，自动重定向至此。隐私政策（`/#/privacy`）与免责声明（`/#/disclaimer`）以该站点内页面展示的最新版本为准，条款更新日期见页内「最后更新」。任何其它域名下的同名页面均非官方，请谨防仿冒。
