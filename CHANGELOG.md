...
- **头部操作改为侧边抽屉**：原头部 6 个操作按钮（语言/主题/导出/导入/设置/锁定）在窄屏会拥挤/截断。改为顶部仅保留「汉堡按钮 + 品牌名」，点击汉堡从左侧滑出抽屉（`van-popup position="left"`，宽 78% / 最大 320px，可滚动），收纳全部操作与法务链接；底部页脚的法务入口保持不变。抽屉样式放入非 scoped `<style>` 块，避免 `van-popup` teleport 到 body 后作用域样式命中不到弹层根节点（浅/深色主题背景失效）。新增 `nav.menu` 中英文案。
