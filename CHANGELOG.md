...
- **头部操作改为侧边抽屉**：原头部 6 个操作按钮（语言/主题/导出/导入/设置/锁定）在窄屏会拥挤/截断。改为顶部仅保留「汉堡按钮 + 品牌名」，点击汉堡从左侧滑出抽屉（`van-popup position="left"`，宽 78% / 最大 320px，可滚动），收纳全部操作；法务入口（隐私政策 / 免责声明）保留在全局页脚，抽屉不再重复。抽屉样式放入非 scoped `<style>` 块，避免 `van-popup` teleport 到 body 后作用域样式命中不到弹层根节点（浅/深色主题背景失效）。新增 `nav.menu` 中英文案。
...
- **文档**：README「安全架构」小节新增「上锁策略」Mermaid 流程图，画出自动锁（推迟）/ 手动锁（确认）双分支及共用的 `editing` 信号来源，说明两级保护设计；同小节追加「导航架构（侧边抽屉）」说明，记录汉堡按钮 + 离屏抽屉方案及兼容性要点（非 scoped 样式 / 内联 SVG 图标 / `safe-area-inset` 刘海屏适配）。
